let arraia = JSON.parse(localStorage.getItem("bd_peça"))
...
for(let i = 0; i < arraia.length; i++){
    ...
    for(let j = 0; j < 4; j++){
        ...
        if(j === 0){
            td.appendChild(document.createTextNode(arraia[i].peça));
        }
        if(j === 1){
            td.appendChild(document.createTextNode(arraia[i].tipo));
        } 
        if(j === 2){
            td.appendChild(document.createTextNode(arraia[i].preço));
        }
        if(j === 3){
            td.appendChild(document.createTextNode(arraia[i].modelo));
        }
    }
}   
}
